<?php
/**
 * Sui Suto - WordPress AI Connector & Block Bindings Integration
 *
 * Integrates the Sui Suto luxury theme with WordPress 7.x core AI layer
 * (AI Client, Connectors API, and Block Bindings), enforcing quiet luxury
...
/**
 * --------------------------------------------------------------------------
 * 1. Brand System Prompt & AI Context
 * --------------------------------------------------------------------------
 * Exposes Sui Suto's luxury brand guidelines, aesthetic rules, and craft glossary
 * to the WordPress AI Client, Connectors, and editorial generation pipelines.
 */
function suisuto_get_ai_brand_context() {
	return array(
		'brand_name'         => 'Sui Suto',
...
/**
 * Filter: Hook into WordPress core AI Client context if available.
 * Also exposed to third-party AI Connectors and Gutenberg Site Editor.
 */
function suisuto_filter_ai_connector_context( $context = array() ) {
	if ( ! is_array( $context ) ) {
		$context = array();
	}
...
add_filter( 'wp_ai_system_instruction', function( $instruction ) {
 ...
 */
function suisuto_register_ai_product_meta() {
	$meta_keys = array(
...
/**
 * --------------------------------------------------------------------------
 * 3. Native Block Bindings API Integration
 * --------------------------------------------------------------------------
 * Binds core Gutenberg blocks (Paragraph, Heading) directly to dynamic
 * textile provenance attributes, allowing AI agents or editors to populate
 * single product templates dynamically.
 */
function suisuto_register_block_bindings() {
	...
	register_block_bindings_source(
		...
		array(
			...
			'get_value_callback' => function( array $source_args, $block_instance ) {
				$post_id = ! empty( $source_args['post_id'] ) ? $source_args['post_id'] : get_the_ID();
				if ( ! $post_id ) {
					return '';
				}
				$craft  = get_post_meta( $post_id, '_suisuto_textile_craft', true );
				$origin = get_post_meta( $post_id, '_suisuto_weave_origin', true );
	...
	register_block_bindings_source(
		...
		array(
			...
			'get_value_callback' => function( array $source_args, $block_instance ) {
				...
				$notes = get_post_meta( $post_id, '_suisuto_artisan_notes', true );
				if ( ! empty( $notes ) ) {
	...
	register_block_bindings_source(
		...
		array(
			...
			'get_value_callback' => function( array $source_args, $block_instance ) {
				...
				$care = get_post_meta( $post_id, '_suisuto_garment_care', true );
...
/**
 * --------------------------------------------------------------------------
 * 4. AI Image Alt-Text Guidance Filter
 * --------------------------------------------------------------------------
 * Hooks into AI media processing to enrich image descriptions with textile terminology.
 */
function suisuto_ai_media_alt_instruction( $instruction, $attachment_id = 0 ) {
	$brand = suisuto_get_ai_brand_context();
